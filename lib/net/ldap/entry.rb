...
class Net::LDAP::Entry
  ...
  # Accesses each of the attributes present in the Entry. Calls a
  # user-supplied block with each attribute in turn, passing two arguments
  # to the block: a Symbol giving the name of the attribute, and a (possibly
  # empty) Array of data values.
  def each
    if block_given?
  ...
  # Converts the Entry to a String, representing the Entry's attributes in
  # LDIF format.
  def to_ldif
    ary = []
    ary << "dn: #{dn}\n"
  ...
  def is_attribute_value_binary?(value)
    v = value.to_s
    return true if v.any? { |byte| (byte < 32) || (byte > 126) }
    if v[0..0] == ':' or v[0..0] == '<'
      return true
    end
    false
